...
if __name__ == "__main__":
    ...
    ###########        # if self.transforms is not None:
        #     pc_np = self.transforms(pc_np)################## automation for ModelNet10 / 40 configuration ####################
    ...
    for epoch in range(args.epochs):
        ...
        if epoch >= 0 and epoch%1==0:
            ...
            if model.test_accuracy.item() > best_accuracy:
                ...
                model.save_network2(model.encoder, 'sonet_enc_stu_best.pth', args.gpu_id)
                model.save_network2(model.classifier, 'sonet_cls_stu_best.pth', args.gpu_id)
            ...
            if model.test_accuracy.item() > saving_acc_threshold:
                ...
                model.save_network(model.encoder, 'encoder_stu', '%d_%f' % (epoch, model.test_accuracy.item()), args.gpu_id)
                model.save_network(model.classifier, 'classifier_stu', '%d_%f' % (epoch, model.test_accuracy.item()), args.gpu_id)
